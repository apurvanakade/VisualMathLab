...
### Chart overlay controls: legend, step/parameter sliders

Every control that's *always-live* (reacts immediately, no Plot/Regenerate button gating it) and whose only job is to change what the chart currently shows — a trace-visibility legend, a step/iteration scrubber, a model parameter like `n` or a filter's α/R/Q — floats directly on the chart instead of sitting in the controls panel below it. This is a uniform layout convention, not a per-page style choice: it's what's left every single `panel-tabset` on the site collapsing (see below), since in every case the second tab turned out to be exactly this category of control.

1. Wrap the page's `mainPlot` cell in `<div class="ojs-plot-overlay">` (`position: relative` in styles.css, the positioning anchor for everything below). Fullscreening a chart via the shared modebar button (`js/plotting/plotly-fullscreen-button.js`) fullscreens this wrapper rather than the bare Plotly div when present, so everything inside it — legend and sliders alike — stays visible in fullscreen.
2. A trace-visibility legend (`viewof <name> = VM.ui.legendOverlay(...)`, see `js/ui/legend-overlay.js` above) is a direct sibling of the `mainPlot` cell inside that wrapper, floating top-left via `.ojs-legend-overlay`. **This replaces Plotly's built-in legend, it doesn't sit alongside it** — every trace `mainPlot` pushes carries `showlegend: false`, so Plotly draws no legend of its own (with two or more traces it otherwise auto-shows one, in Plotly's default style, in the top-right corner — visibly not this site's chrome). The wiring, copied verbatim across pages (`root-finding/newton-method/index.qmd` is the reference): a `traceOptions` cell listing the labels in display order, a `traceColors` `Map` from label → the exact color `mainPlot` draws that trace in (so the swatches double as a color key), one `show<Trace> = traces.includes("<label>")` `//| output: false` cell per trace, and `mainPlot(setup, ...show<Trace>)` gating each `data.push` on its flag — a hidden trace is never pushed, rather than pushed with `visible: false`.
3. Every always-live slider (and any text field tightly paired with one, e.g. a "Max" bound on a slider's range) is a further sibling, wrapped in `<div class="ojs-step-overlay">` (styles.css) — the same floating-panel chrome as the legend, but anchored to the bottom edge and spanning the chart's width instead of hugging a corner.
   - One or two controls (e.g. root-finding's Step + Max, `differential-equations/*`'s `n`/tolerance + Max): combine with `.ojs-row` (`<div class="ojs-step-overlay ojs-row">`) so the first control fills the remaining width and the second stays at its natural size — the same `.ojs-row` primitive used everywhere else, just relocated.
   - Three or more controls (e.g. `probability/recursive-filters/1d-recursive-filters/index.qmd`'s four filter-parameter sliders): combine with `.ojs-grid` instead (`<div class="ojs-step-overlay ojs-grid">`), which auto-wraps into multiple rows on narrow viewports rather than cramming everything into one line.
   - Any `//| output: false` helper cell these sliders depend on (a `maxN`/`tolerance`/`maxSteps` parser) is declared just above `.ojs-plot-overlay` opens, not inside the row/grid div — an output:false cell placed inside an `.ojs-row`/`.ojs-grid` still emits a real (zero-size) `.cell` node that counts as a flex/grid child and steals the first-child/last-child (or column) layout meant for a visible sibling.
4. `data-example-field` tagging still works with a field in the overlay exactly as it does for a field in the panel — `document.querySelector` finds it in the DOM regardless of where it sits (see the example-dropdown wiring below).

**A page's remaining controls panel holds only what's left**: text fields gated behind a commit button (function, endpoints, initial conditions), the example dropdown, the Plot/Regenerate button, and any button-triggered action (e.g. `model-fitting/l1-vs-l2/index.qmd`'s "Regenerate points"). If that's everything the page has, it's a single group — one plain `ojs-panel`, per the single-group rule below.

### Tabbed controls

`::: {.panel-tabset}` (Quarto/Bootstrap nav-tabs) is available for when a page's controls form two or more distinct groups that *aren't* reducible to "static panel fields + always-live chart-overlay controls" above — but as of this writing no page on the site actually needs it: every prior tabset (`differential-equations/explicit-methods/index.qmd`'s "Inputs"/"Display Options", `probability/recursive-filters/1d-recursive-filters/index.qmd`'s "Signal & Sampling"/"Filter Parameters", every `root-finding/*` page, etc.) turned out to be exactly "panel fields" vs. "sliders that belong on the chart," and collapsed to a single `ojs-panel` once those sliders moved to the overlay. Before reaching for a tabset on a new page, check whether the second group is actually just overlay-eligible sliders/legend toggles — if so, use the overlay pattern above instead, not a tab. A page whose controls are genuinely a single group (e.g. `model-fitting/l1-vs-l2/index.qmd`, `differential-equations/butcher-tableau/index.qmd`, and now every other interactive page on the site) has nothing to split and stays as one plain `ojs-panel`; don't force a single-tab tabset just for consistency's sake.
