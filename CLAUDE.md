...
Most computational-math pages (e.g. `root-finding/newton-method/index.qmd`, `piecewise-interpolation/integration/index.qmd`) make their state shareable via the URL's query string, and pair that with a gallery of preset links at the bottom of the page:
...
3. An `## Examples` section near the end of the page holds a `<div class="example-grid">` of `<a class="example-card" href="?f=...&...">...</a>` links (see `styles.css`'s `.example-grid`/`.example-card` rules) — each is a title + formula + one-sentence description, and clicking it reloads the page with those query params, which step 1's input cells then pick up as their new defaults. Follow this pattern for any new method page that takes function/parameter text input; it's what makes "here's a good example of this method" linkable and shareable instead of requiring manual field entry.

**Every input on the page must be wired into the URL whenever possible** — not just the function/endpoint fields gated behind the commit button. Step counts, method checkboxes, and any other always-live control need both an `initial<X>` cell reading it back out (step 1) and an entry in `committed`/`urlSyncControls` writing it back (step 2), the same as `piecewise-interpolation/integration/index.qmd`'s `initialN`/`initialMax`/`initialMethods` + `urlSyncControls`. For a checkbox group, add a slug↔label `Map` pair (`methodSlugToLabel`/`methodLabelToSlug`) so the URL uses short stable slugs instead of the human-readable labels verbatim. An example card's `href` is a contract: every query param it sets must actually be read by an input cell, or the example silently doesn't do what it claims. When adding or editing an `## Examples` section, grep the page for `params.get`/`urlParams.get` and confirm every param named in an `href` has a matching read — `differential-equations/explicit-methods/index.qmd` shipped for a while with `n`/`max`/`show` params on every example card that no cell ever read.
...
- Interactive pages reuse the design-system CSS classes in `styles.css` (`ojs-panel`, `ojs-row`, `ojs-grid`, `ojs-fill`, `ojs-auto`, `ojs-table-container`, `ojs-table-toolbar`, `plotly-box-large`, `example-grid`, `example-card`) rather than ad hoc per-page styling. `ojs-panel` is chrome only; `ojs-row`/`ojs-grid` are layout only — combine them (`class="ojs-panel ojs-row"`) for a standalone panel, or nest a bare `ojs-row`/`ojs-grid` inside an `ojs-panel` for a control group within a larger panel. Attach `ojs-fill`/`ojs-auto` via `classList.add` on an `Inputs.*` wrapper div to make it fill its row or keep its natural width, instead of setting `el.style` directly. `example-card` nests `example-card-title`/`example-card-formula`/`example-card-text` divs — see the URL-shareable examples pattern above.
