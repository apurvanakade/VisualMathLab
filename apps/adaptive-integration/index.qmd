...
viewof maxDepthInput = Inputs.text({value: "15", label: "Max depth", placeholder: "Example: 15, 5*3"})
```
...
**Adaptive quadrature** refines the partition of $[a,b]$ only where it's needed, instead of the fixed grid used in [Numerical Integration](../numerical-integration/index.qmd). Simpson's rule on an interval $[p,q]$ with midpoint $m=(p+q)/2$,
