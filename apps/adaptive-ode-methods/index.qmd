...
```{ojs}
viewof tolExponent = {
  const tolSlider = Inputs.range([0, 10], {step: 0.5, value: 0, label: "Tolerance exponent e (ε = 10⁻ᵉ)"})
...
The [explicit methods](../explicit-ode-methods/index.qmd) take every step with the same size $h$. An **adaptive** method instead picks a new $h$ at each step: small where the solution is changing quickly, large where it is nearly straight.
