...
date: 2026-07-13
order: 3
image: adaptive-ode-methods.png
categories:
  - Computational Math
...
examples = [
  ...
  },
  {
    ...
    params: {f: "-2 * y", y0: "3", b: "4"}
  },
  {
    title: "Exponential growth",
    params: {f: "2 * y", y0: "0.25", b: "2"}
  },
  {
    title: "Damped sinusoidal forcing",
    params: {f: "sin(t) - 0.5 * y", y0: "0", b: "10"}
  },
  {
  ...
  },
  {
    ...
    params: {f: "-10 * (y - cos(t)) - sin(t)", y0: "1", b: "3"}
  },
  {
...
// field overlaid on the chart are unaffected and stay reactive.
viewof plotTrigger = {
...
viewof maxStepsInput = Inputs.text({value: "2000", label: "Max steps", placeholder: "Example: 2000, 1000*2"})
```
...
```{ojs}
//| output: false
showAdaptive = methods.includes("Adaptive RK45")
```
...
```{ojs}
mainPlot(setup, showRefSolution, showAdaptive)
```
...
incrementsPlot = {
  ...
  const colorRange = [chartColors.fn, chartColors.alt]
  ...
  const chart = Plot.plot({
    ...
    marks: [
      ...
      Plot.ruleY([0], {stroke: chartColors.muted, strokeWidth: 1}),
...
**RKF45** (Runge–Kutta–Fehlberg) adapts the step size $h$ at every step, instead of the fixed $h=b/n$ used in [Explicit Methods](../explicit-ode-methods/index.qmd). Six slope evaluations $k_1,\dots,k_6$, each of the general Runge-Kutta form
