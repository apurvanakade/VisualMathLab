...
examples = [
  {
    ...
    params: {f: "-10 * (y - cos(t)) - sin(t)", y0: "0", b: "3"}
  },
  {
...
viewof y0 = {
  const params = new URLSearchParams(window.location.search)
  const y0Input = Inputs.text({label: "Initial condition y(0)", value: params.get("y0") ?? "0", placeholder: "Example: 0.5, 1/2"})
...
viewof bEnd = {
  const params = new URLSearchParams(window.location.search)
  const bEndInput = Inputs.text({label: "Right endpoint b", value: params.get("b") ?? "3", placeholder: "Example: 2, pi"})
