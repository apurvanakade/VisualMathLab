...
/* Both chart panels hide themselves when they have nothing in them: the
   ...
   takes no flow space at all -- it sits above the chart, not over it. */
.ojs-chart-controls:not(:has(input[type="range"])) {
  display: none;
}

/* :not(:has(a row)) rather than :empty -- draggable-overlay.js appends its
   drag grip to the legend panel, so a legend with no rows still has a
...
/* The site's grid slider bar expects one cell per slider, so each becomes a
   ...
   cell's output in several nested divs, so .ojs-chart-controls's own grid sees
...
.ojs-chart-controls .vm-params-form {
  display: grid;
