...
/* The shape key. Same box the diagram draws, at the same two corner radii,
   so the swatch is the thing it stands for rather than a description of it. */
.ojs-graph-legend-box {
  display: inline-block;
  width: 1.5rem;
  height: 0.85rem;
...
.ojs-graph-legend-box.is-discrete { border-radius: 2px; }

.ojs-graph-legend-box.is-continuous { border-radius: 7px; }
...
Solid edges are exact relationships — a special case, a sum, or a transformation; dashed edges are limiting relationships that only hold as a parameter grows large. Square boxes are discrete distributions, rounded ones continuous.
...
<span><span class="ojs-graph-legend-box is-discrete"></span>Discrete</span>
<span><span class="ojs-graph-legend-box is-continuous"></span>Continuous</span>
</div>
