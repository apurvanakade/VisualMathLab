...
```{ojs}
//| output: false
// VM is the shared utility library, loaded globally via _includes/head-scripts.html (js/**). This page uses VM.numerical.polynomialFit, VM.plotting.paddedRange, VM.ui.renderTable, VM.ui.applyExampleParams.
VM = window.VM
```
...
initialNoise = {
  const raw = Number(urlParams.get("noise"))
  return Number.isFinite(raw) ? Math.max(0, Math.min(maxNoise, raw)) : 0
}

initialSeed = {
  const raw = Number(urlParams.get("seed"))
  return Number.isInteger(raw) && raw >= 0 ? raw : 123
}

initialShownDegrees = {
  const raw = urlParams.get("degrees")
  if (!raw) return ["Degree 5"]
  ...
  return labels.length ? labels : ["Degree 5"]
}
```
...
// "degrees" holds the actual checkbox option labels (matching what
// Inputs.checkbox's view expects), not the URL's comma-separated numbers.
examples = [
  {
    title: "No noise",
    params: {noise: 0, degrees: ["Degree 1", "Degree 5"], seed: 101}
  },
  {
    title: "Small noise",
    params: {noise: 0.25, degrees: ["Degree 1", "Degree 2", "Degree 5"], seed: 202}
  },
  {
    title: "Underfitting vs overfitting",
    params: {noise: 0.7, degrees: ["Degree 1", "Degree 5"], seed: 303}
  },
  {
    title: "Training error decreases",
    params: {noise: 1, degrees: ["Degree 1", "Degree 2", "Degree 3", "Degree 4", "Degree 5"], seed: 404}
  },
  {
    title: "Degree 5 interpolation",
    params: {noise: 1, degrees: ["Degree 5"], seed: 505}
  },
  {
    title: "Same data, linear model",
    params: {noise: 1, degrees: ["Degree 1"], seed: 505}
  }
]
```
...
// counts as one draw). It is deterministic from the current seed plus the
// regenerate count, so URL example cards always produce the same first draw,
// while "Regenerate points" still produces a new reproducible draw.
// All noise comes from `offsets`, scaled by the noise slider in `points`
// below — so the points are exactly colinear at noise level 0, on whatever
// line this draw picked.
...
basePoints = {
  ...
  // Seeded PRNG (mulberry32). This replaces Date.now() so examples are
  // reproducible from URL parameters.
  let state = Math.floor(seedValue) + regenerateTrigger * 1000003
  const rand = () => {
...
viewof noiseLevel = {
  ...
  input.dataset.exampleField = "noise"
  return input
}
```
...
exampleFieldSelectors = ({
  noise: '[data-example-field="noise"]',
  degrees: '[data-example-field="degrees"]',
...
// no page reload, so no scroll jump either. noiseLevel/shownDegrees/
// seedValue are all fully live/reactive (no commit-button gate), so no
// trigger click is actually needed for the values to take effect; a
// harmless no-op element (with no <button> child) is passed as the
// required triggerEl argument instead of "Regenerate points" — clicking
// Regenerate would bump regenerateTrigger, which basePoints also mixes
// into its seed, producing a different point draw than the one the
// example's seed was chosen to reproduce. A plain "change" listener
...
viewof shownDegrees = {
  ...
  const checkboxInput = Inputs.checkbox(options, {value: initialShownDegrees})
  checkboxInput.classList.add("ojs-fill")
  checkboxInput.dataset.exampleField = "degrees"
