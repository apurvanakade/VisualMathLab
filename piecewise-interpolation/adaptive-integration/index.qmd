...
**Adaptive quadrature** refines the partition of $[a,b]$ only where it's needed, instead of the fixed grid used in [Numerical Integration](../integration/index.qmd): it compares Simpson's rule on $[a,b]$ against Simpson's rule on each half, and bisects further whenever the two disagree by more than a target tolerance $\varepsilon$.
