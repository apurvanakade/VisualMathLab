...
traceColors = new Map([
  ["f(x)", "#2563eb"],
  ["Adaptive Simpson panels", "#16a34a"],
  ["Partition points", "#111827"]
])
```
...
mainPlot = {
  ...
  return (data, showFCurve, showPanels, showPartitionPoints) => {
    ...
    if (showPartitionPoints && data.nodesX.length > 0) {
      traces.push({x: data.nodesX, y: data.nodesY, type: "scatter", mode: "markers", name: "Partition points", showlegend: false, marker: {color: "#111827", size: 6, symbol: "circle", line: {color: "white", width: 1}}})
    }
