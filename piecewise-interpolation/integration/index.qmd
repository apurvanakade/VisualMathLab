...
methodColors = new Map([
  ...
  ["Midpoint Riemann sum", "#f59e0b"],
  ["Trapezoidal rule", "#dc2626"],
  ["Simpson's rule", "#16a34a"],
  ["Interpolation nodes", "#111827"]
])
...
// Inputs and Display Options tabs) and clicks Plot, in place — no page
// reload, so no scroll jump either. A plain "change" listener attached
...
mainPlot = {
  ...
  return (data, showFCurve, showLeftRiemann, showMidpoint, showTrapezoidal, showSimpson, showNodes) => {
    ...
    if (showFCurve) {
      traces.push({x: data.sampleXs, y: data.sampleYs, type: "scatter", mode: "lines", name: "f(x)", showlegend: false, line: {color: "#2563eb", width: 3}})
    }
    ...
    if (showNodes && data.nodesX.length > 0) {
      traces.push({x: data.nodesX, y: data.nodesY, type: "scatter", mode: "markers", name: "Interpolation nodes", showlegend: false, marker: {color: "#111827", size: 7, symbol: "circle", line: {color: "white", width: 1}}})
    }
