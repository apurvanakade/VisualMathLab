...
initialInterpolants = {
  ...
  const all = ["Original curve", "Linear interpolant", "Cubic spline interpolant"]
  if (!raw) return all
...
examples = [
  {
    ...
    params: {f: "sin(x)", a: "0", b: "2*pi", max: "20", n: "4", show: ["Original curve", "Linear interpolant", "Cubic spline interpolant"]}
  },
  {
    title: "Cosine on half a period",
    params: {f: "cos(x)", a: "0", b: "pi", max: "24", n: "3", show: ["Original curve", "Linear interpolant", "Cubic spline interpolant"]}
  },
  {
    title: "Cubic polynomial",
    params: {f: "x^3-x", a: "-2", b: "2", max: "30", n: "4", show: ["Original curve", "Linear interpolant", "Cubic spline interpolant"]}
  },
  {
    title: "Rational peak",
    params: {f: "1/(1+x^2)", a: "-5", b: "5", max: "40", n: "6", show: ["Original curve", "Linear interpolant", "Cubic spline interpolant"]}
  },
  {
    title: "Gaussian bump",
    params: {f: "exp(-x^2)", a: "-3", b: "3", max: "35", n: "5", show: ["Original curve", "Linear interpolant", "Cubic spline interpolant"]}
  },
  {
    title: "Nonsmooth corner",
    params: {f: "abs(x)", a: "-2", b: "2", max: "30", n: "4", show: ["Original curve", "Linear interpolant", "Cubic spline interpolant"]}
  }
]
```
...
```{ojs}
viewof interpolants = {
  const options = ["Original curve", "Linear interpolant", "Cubic spline interpolant"]
  const checkboxInput = Inputs.checkbox(options, {value: initialInterpolants})
...
mainPlot = {
  ...
  return (data, showOriginal, showLinear, showCubic) => {
    const traces = []

    if (showOriginal) {
      traces.push({x: data.sampleXs, y: data.sampleYs, type: "scatter", mode: "lines", name: "f(x)", showlegend: true, line: {color: "#2563eb", width: 3}})
    }
...
```{ojs}
mainPlot(setup, showOriginal, showLinear, showCubic)
```
