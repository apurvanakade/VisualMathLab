...
initialMaxNText = urlParams.get("max") ?? "20"
...
interpolantColors = new Map([
  ["Original curve", "#2563eb"],
  ["Linear interpolant", "#dc2626"],
  ["Cubic spline interpolant", "#16a34a"],
...
examples = [
  ...
  {
    title: "Cosine on half a period",
    params: {f: "cos(x)", a: "0", b: "pi", max: "24", n: "3", show: ["Original curve", "Linear interpolant", "Cubic spline interpolant", "Interpolation nodes"]}
  },
  {
  ...
  {
    title: "Rational peak",
    params: {f: "1/(1+x^2)", a: "-5", b: "5", max: "40", n: "6", show: ["Original curve", "Linear interpolant", "Cubic spline interpolant", "Interpolation nodes"]}
  },
  {
    title: "Gaussian bump",
    params: {f: "exp(-x^2)", a: "-3", b: "3", max: "35", n: "5", show: ["Original curve", "Linear interpolant", "Cubic spline interpolant", "Interpolation nodes"]}
  },
  {
...
```{ojs}
viewof maxNInput = {
  const input = Inputs.text({value: initialMaxNText, label: "Max", placeholder: "Example: 20, 5*4"})
...
mainPlot = {
  ...
  return (data, showOriginal, showLinear, showCubic, showNodes) => {
    ...
    if (showLinear && data.linearX.length > 0) {
      traces.push({x: data.linearX, y: data.linearY, type: "scatter", mode: "lines", name: "Linear interpolant", showlegend: false, line: {color: "#dc2626", width: 2.5}})
    }
    if (showCubic && data.cubicX.length > 0) {
      traces.push({x: data.cubicX, y: data.cubicY, type: "scatter", mode: "lines", name: "Cubic spline interpolant", showlegend: false, line: {color: "#16a34a", width: 2.5}})
    }
    if (showNodes && data.nodesX.length > 0) {
      traces.push({x: data.nodesX, y: data.nodesY, type: "scatter", mode: "markers", name: "Interpolation nodes", showlegend: false, marker: {color: "#111827", size: 8, symbol: "circle", line: {color: "white", width: 1}}})
    }
