...
//   xt      parametric x(t)
//   yt      parametric y(t)
//   sigma   measurement noise, a fixed absolute standard deviation (independent on x and y)
...
viewof sigmaText = {
  const params = new URLSearchParams(window.location.search)
  const input = Inputs.text({label: "Noise σ", value: params.get("sigma") ?? "0.1", placeholder: "Example: 0.1, 1/2"})
  return input
}
```
...
sim = {
  ...
  // The filter starts from a zero-velocity, uninformative-prior guess (see
  // INITIAL_VELOCITY_VARIANCE), so it needs a handful of samples to learn
  // the true velocity before its estimate settles down -- on a fast
  // trajectory this startup transient alone can briefly push the relative
  // error above RELATIVE_ERROR_LIMIT (denominator still near its floor of
  // 1 this early on) even for a well-tuned filter. The divergence guard is
  // skipped for the first few samples after each Reset so this normal,
  // self-correcting startup wobble is never mistaken for real divergence.
  const STARTUP_GRACE_SAMPLES = 5
  ...
  const resetSession = () => {
    ...
    const p0 = committed.sigma > 0 ? committed.sigma * committed.sigma : 0.01
  ...
  const generateSample = () => {
    ...
    // Noise is a fixed absolute size, the same regardless of how far the
    // particle currently is from the origin.
    const noisyX = trueX + committed.sigma * sess.gaussian()
    const noisyY = trueY + committed.sigma * sess.gaussian()
    ...
    const pastStartup = sampleIndex >= STARTUP_GRACE_SAMPLES
    sess.diverged = !Number.isFinite(trueX) || !Number.isFinite(trueY) || !Number.isFinite(relErr) || (pastStartup && relErr > RELATIVE_ERROR_LIMIT)
...
<div class="example-grid">
  ...
  <a class="example-card" href="?xt=10*t&yt=5*t&sigma=0.4&seed=1&R=0.16&Q=0.02&tab=filters">
    <div class="example-card-title">Straight line -- well-tuned</div>
    <div class="example-card-formula">x(t) = 10t, y(t) = 5t -- zero acceleration</div>
    <div class="example-card-text">Constant velocity matches the filter's own model exactly; R matched to the noise level tracks it tightly, with no acceleration to ever lag behind.</div>
  </a>

  <a class="example-card" href="?xt=10*t&yt=5*t&sigma=0.4&seed=1&R=0.0005&Q=0.02&tab=filters">
    <div class="example-card-title">Straight line -- R too low</div>
    <div class="example-card-formula">Same line and noise, R = 0.0005 instead of 0.16</div>
    <div class="example-card-text">Same measurements as the well-tuned line above, but R this small tells the filter to trust each noisy measurement almost exactly -- the estimate spreads across the noise instead of settling onto the line.</div>
  </a>

  <a class="example-card" href="?xt=cos(10*t)&yt=sin(10*t)&sigma=0.08&seed=2&R=0.006&Q=0.05&tab=filters">
    <div class="example-card-title">Circular orbit -- well-tuned</div>
    <div class="example-card-formula">x(t) = cos(10t), y(t) = sin(10t) -- constant nonzero acceleration</div>
  ...
  <a class="example-card" href="?xt=cos(10*t)&yt=sin(10*t)&sigma=0.08&seed=2&R=0.006&Q=0.0005&tab=filters">
    ...
    <div class="example-card-text">Q this small can't keep re-adjusting to the circle's constant turning -- the estimate lags and drifts off the true path.</div>
  </a>

  <a class="example-card" href="?xt=10*t*cos(10*t)&yt=10*t*sin(10*t)&sigma=0.3&seed=3&R=0.09&Q=0.3&tab=filters">
    <div class="example-card-title">Outward spiral -- well-tuned</div>
    <div class="example-card-formula">x(t) = 10t·cos(10t), y(t) = 10t·sin(10t) -- growing acceleration</div>
  ...
  <a class="example-card" href="?xt=10*t*cos(10*t)&yt=10*t*sin(10*t)&sigma=0.3&seed=3&R=0.09&Q=0&tab=filters">
