...
result = {
  ...
  if (!f) return {rows: [], f: fallback, status: "invalid-function"}

  let a = Number(committed.a0)
  let b = Number(committed.b0)

  if (!Number.isFinite(a) || !Number.isFinite(b) || a === b) {
    return {rows: [], f, status: "invalid-interval"}
  }
  ...
  if (!Number.isFinite(fa) || !Number.isFinite(fb)) {
    return {rows: [], f, status: "nonfinite-endpoint"}
  }
  ...
  if (fa * fb > 0) {
    return {rows: [], f, status: "no-sign-change"}
  }
  ...
  while (n < maxIterations) {
    ...
    if (!Number.isFinite(fm)) {
      return {rows, f, status: "nonfinite-midpoint"}
    }

    // If the midpoint is exactly a root, stop.
    if (fm === 0) {
      return {rows, f, status: "midpoint-root"}
    }
  ...
  return {rows, f, status: "max-iterations"}
}
```
