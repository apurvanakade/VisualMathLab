...
```{ojs}
//| output: false
// Labels for the ::: {.panel-tabset} below, kept in sync with a `tab` URL
// param by the tabSync cell further down.
...
traceOptions = ["f(x)", "y = 0", "Secant line", "Secant points", "Secant x-intercepts"]
...
traceColors = new Map([
  ...
  ["y = 0", "#111827"],
  ["Secant line", "#dc2626"],
  ["Secant points", "#dc2626"],
  ["Secant x-intercepts", "#16a34a"]
])
```
...
tabSync = {
  ...
  const initialLabel = tabSlugToLabel.get(initialParams.get("tab"))
  if (initialLabel) {
...
```{ojs}
//| output: false
showZeroLine = traces.includes("y = 0")
```
...
mainPlot = {
  ...
  return ({visibleRows, xRange, xBuffer, yRange, sampleXs, sampleYs}, showFCurve, showZeroLine, showSecant, showSecantPoints, showIntercepts) => {
    const data = []
    ...
    if (showZeroLine) {
      ...
      data.push({x: [xBuffer.lo, xBuffer.hi], y: [0, 0], type: "scatter", mode: "lines", name: "y = 0", showlegend: false, line: {color: "#111827", width: 2}})
    }
...
```{ojs}
mainPlot(setup, showFCurve, showZeroLine, showSecant, showSecantPoints, showIntercepts)
```
